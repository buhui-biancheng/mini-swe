# -*- coding: utf-8 -*-
"""官方模式冒烟：pytest-dev__pytest-7168（与 Claude Code 子 agent 双跑对比）。
...
BUDGET = 3_000_000  # 2026-08-14 优化版冒烟：pytest 上限 300 万（Claude Code 58 万×5≈2.9M）

STATUS = "/home/yuanyin292/桌面/xiangmu1/eval/official_pytest7168_status.json"
RESULT = "/home/yuanyin292/桌面/xiangmu1/eval/official_pytest7168_result.json"
...
def main():
    ...
    if _rel_tests:
        ...
        # pytest 自举：-c /dev/null 绕 tox.ini，-p pytester 加载内置插件
        test_cmd = (f"cd /workspace && {src_hint}python3 -m pytest {_tf} "
                    f"-q -p no:cacheprovider -c /dev/null -p pytester")
    else:
        test_cmd = f"cd /workspace && {src_hint}python3 -m pytest {test_dir} -q -p no:cacheprovider -c /dev/null -p pytester"
