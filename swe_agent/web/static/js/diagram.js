...
function animateStage(ev){
  ...
  if (ev.type === "turn_start"){ if (typeof fsmReset === "function") fsmReset(); return; }  // 新任务重置状态图
  if (ev.type === "state"){ if (typeof animateFSM === "function") animateFSM(ev); return; } // FSM 状态流转
  const spec = STAGE[ev.type];
  if (!spec || !document.querySelector(".arch.arch-diagram")) return;  // archSVG 已裁剪，waku 架构动画停用
