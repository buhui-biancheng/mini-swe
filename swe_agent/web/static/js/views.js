...
const VIEWS = {
  ...
  overview(d){
    ...
    const g = d.graph || {node_count:0};
    const tiles = [
        [s.turns,"修复任务",""],[s.tool_calls,"工具调用",""],
        [s.tool_errors,"工具错误",""],[secs(s.latency_avg),"平均耗时",""],
        [g.node_count||0,"图节点",""],[d.turns.length?d.turns[0].iterations??0:0,"最近迭代",""],
      ].map(([v,l,c])=>`<div class="tile"><b class="${c}">${v}</b><span>${l}</span></div>`).join("");
    return `<div class="tiles">${tiles}</div>
    <h2>FSM 状态流转 — 实时 <span class="arch-status"></span></h2>
    ${fsmPanel()}
    <h2>代码依赖图 — AI 读取的节点实时点亮 <span class="arch-status"></span></h2>
    ${graphPanel(d)}
    <h2>最近一次修复</h2>${d.turns.length?turnCard(d.turns[0]):'<div class="card empty">还没有运行过修复 — 在右侧输入 bug 文件路径开始</div>'}`;
  },
  loop(d){
  ...
  graph(d){
    ...
    let h = `<h2>FSM 状态流转 — 实时 <span class="arch-status"></span></h2>${fsmPanel()}`;
    h += `<div class="meta" style="margin-bottom:14px">The loop is one agent turn: the model picks tools until
